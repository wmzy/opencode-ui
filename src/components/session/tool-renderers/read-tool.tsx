...
export function ReadToolRenderer({ input, output, status, error, defaultOpen = false }: ToolRendererProps) {
  ...
  const subtitle = isPending ? 'Reading...' : (filePath.split('/').pop() ?? filePath);
  ...
  const isTruncated = output ? output.split('\n').length > MAX_PREVIEW_LINES : false;

  return (
    <div className={wrapperStyle}>
      <button className={triggerStyle} onClick={handleToggle} aria-expanded={open}>
        {statusIcon}
        <span className={toolNameStyle}>Read</span>
      ...
      <div className={cx(contentStyle, !open && contentCollapsedStyle)}>
        ...
        {previewLines ? (
          <>
            <div className={previewStyle}>{previewLines.join('\n')}</div>
            {isTruncated && (
              <div style={{ fontSize: 11, color: 'var(--color-text-tertiary)', marginTop: 4 }}>
                Preview truncated. Full output available in expanded view.
              </div>
            )}
          </>
        ) : output ? (
